...
int servo_move(float degrees) {
    ...
    // Calculate the wait time based on the difference between current and previous degrees
    static float previous_degree = 180.0;  // Initial position is 90 degrees (center)
    float wait_time = abs(degrees - previous_degree) * 10; // Adjust multiplier for servo speed
    timer_waitMillis(wait_time); // Wait for the servo to reach the desired position

    lcd_printf("Pos: %.1f\nMatch: %u", degrees, match_value);

    // Update previous_degree to the current degree after moving
